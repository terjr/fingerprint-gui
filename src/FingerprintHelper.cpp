...
bool requestFingerprint(int pipe_w,const char *display,char *service,char *username,int argc,char **argv){
    int lastResult;
...
//    handler=&deviceHandler;
    ...
    syslog(LOG_DEBUG,"Have display=%s, XAUTHORITY=%s.",display,getenv("XAUTHORITY"));
    ...
    if(username==NULL){             // user not known yet, maybe it's a login
        if(mode==MODE_IDENTIFY){    // device can identify. So identifying the user is possible
            ...
            for(;devices!=NULL;devices=devices->next){
                ...
                FingerprintDiscoverer discoverer(devices);
    ...
    for(;devices!=NULL;devices=devices->next){
        ...
        FingerprintDiscoverer discoverer(devices,username);
        identifyData=discoverer.getIdentifyData();
...
// arg[3] -- DISPLAY variable or empty string
...
int main(int argc,char** argv) {
    ...
    syslog(LOG_ERR,"Started with UID %d, EUID %d.",getuid(),geteuid());
    ...
    if(pidfile){
        ...
        if(i!=getpid()){
            ...
            waitpid(i,&rc,0);
	    unlink(HELPER_PID);
            usleep(500000);   // Wait for devices to stop
        }
        fclose(pidfile);
    }
    ...
    if(!requestFingerprint(pipe_w,display,service,username,argc,argv)){
...
// Try to get a saved password from external media (USB stick)
    char *savedPassword;
    // Create a "UserSettings" object
    UserSettings userSettings((char*)user);
