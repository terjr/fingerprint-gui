...
void MainWindowImpl::savePasswd(){
    ...
    if(!userSettings.writeConfig((char*)passwdPasswd.toStdString().data())){
...
void MainWindowImpl::acquireFinished(int result){
    movie->setPaused(true);
    if(result==RESULT_ENROLL_COMPLETE)
        sDialog=new SavedDialogImpl(MODE_SAVED,this);
    else
        sDialog=new SavedDialogImpl(MODE_NOT_SAVED,this);
    sDialog->show();
    connect(sDialog,SIGNAL(choice(savedDialogChoice)),this,SLOT(savedDialogChosen(savedDialogChoice)));
}

void MainWindowImpl::verifyFinished(int result){
    movie->setPaused(true);
    if(result==RESULT_VERIFY_MATCH)
        sDialog=new SavedDialogImpl(MODE_VERIFIED,this);
    else
        sDialog=new SavedDialogImpl(MODE_NOT_VERIFIED,this);
    sDialog->show();
    connect(sDialog,SIGNAL(choice(savedDialogChoice)),this,SLOT(savedDialogChosen(savedDialogChoice)));
}
...
void MainWindowImpl::scanTabToBack(){
    ...
    if(currentFingerprint!=NULL){
        ...
        disconnect(currentFingerprint->getDevice(),SIGNAL(verifyResult(int)),currentFingerprint,SLOT(newVerifyResult(int)));
        disconnect(currentFingerprint,SIGNAL(acquireFinished(int)),this,SLOT(acquireFinished(int)));
        disconnect(currentFingerprint,SIGNAL(verifyFinished(int)),this,SLOT(verifyFinished(int)));
        delete(currentFingerprint);
        currentFingerprint=NULL;
    }
...
void MainWindowImpl::scanTabToFront(){
    ...
    disconnect(currentFingerprint->getDevice(),SIGNAL(verifyResult(int)),currentFingerprint,SLOT(newVerifyResult(int)));
    connect(currentFingerprint->getDevice(),SIGNAL(verifyResult(int)),currentFingerprint,SLOT(newVerifyResult(int)));
    disconnect(currentFingerprint,SIGNAL(acquireFinished(int)),this,SLOT(acquireFinished(int)));
    connect(currentFingerprint,SIGNAL(acquireFinished(int)),this,SLOT(acquireFinished(int)));
    disconnect(currentFingerprint,SIGNAL(verifyFinished(int)),this,SLOT(verifyFinished(int)));
    connect(currentFingerprint,SIGNAL(verifyFinished(int)),this,SLOT(verifyFinished(int)));
...
void MainWindowImpl::passwordTabToFront(){
    ...
    UserSettings us((char*)username.toStdString().data());
