...
UserSettings::UserSettings(char *userName){
    PermissionHandler ph;
    providePassword=false;
    umountRequired=false;
    if(!userName)return;
    if(strlen(userName)<1)return;
    pathToConfig=QString(DATA_DIR);
    pathToConfig.append(userName);
    pathToConfig.append("/config.xml");

    string user(userName);
    string path(pathToConfig.toStdString());
    // Check permissions of config.xml
    if(!ph.checkPermissions(&path,&user))
        return;
...
bool UserSettings::writeConfig(char *passwd){
    PermissionHandler ph;
    providePassword=false;
    QFile configFile(pathToConfig);
    ...
    string path(pathToConfig.toStdString());
    if(!ph.setPermissions(&path,NULL))
        return false;
