    ...
    // discover devices handled by "libbsapi" from UPEK
        ...
        if(bsapiHandle){
            if((*bsapiDiscoverFunction)("usb",&discoveredBsDevices)==ABS_STATUS_OK){
	            ...
	            syslog(LOG_ERR,"No devices found by libbsapi.");
		}
            }
            else{
...
int DeviceHandler::release(){
    ...
    syslog(LOG_ERR,"Devices released.");
    return -1;
}
...
int DeviceHandler::initialize(){
    ...
    syslog(LOG_ERR,"Devices initialized.");
    return -1;
}
...
void DeviceHandler::upekLoader(){
    ...
    if(bsapiHandle){
        syslog(LOG_INFO,"Proprietary lib \"%s\" loaded.",BSAPI_LIBNAME);
        *(void **)(&bsapiInitFunction)=dlsym(bsapiHandle,BSAPI_INIT);
        ...
        syslog(LOG_INFO,"Proprietary lib \"%s\" not found in library path.",BSAPI_LIBNAME);
    }
}
