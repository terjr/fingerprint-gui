...
UpekDevice::UpekDevice(void *libHandle,struct abs_device_list_item *bs,USBDevice *knownUSBDevices){
    char *error;
    bsData=NULL;
    conn=0;
    absFlag=0;
...
//    ABS_SAMPLE_IMAGE *bsImage;
    ...
    if(!bsapiVerifyFunction){
        emit verifyResult(RESULT_VERIFY_NO_MATCH);
        return false;
    }
...
//        syslog(LOG_DEBUG,"Verify: grab image end.");
    ...
    if(bsResult==0){
        ...
        emit verifyResult(RESULT_VERIFY_MATCH);
        return true;
    }
