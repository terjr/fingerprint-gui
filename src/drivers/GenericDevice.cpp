...
int GenericDevice::getData(void **d){
    int size=0;
...
void GenericDevice::stop(){
    if(dev!=NULL){
        ...
        // because of the missing "cancelOperation" call to libfprint devices:
...
bool GenericDevice::verify(){
    ...
    do{
        emit verifyResult(RESULT_SWIPE);
...
//            }
        ...
        emit verifyResult(translateVerify(result));
...
bool GenericDevice::identify(){
    ...
    int result=fp_identify_finger(dev,identifyData,(size_t*)&match);
    if(result<0){
